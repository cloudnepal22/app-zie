<?php
	...
	if(isset($_POST['perbarui'])){
		...
		if($sizeform>$maxsize){
				echo "<script>alert('Formulir tidak boleh lebih dari 2MB!');</script>";
				echo "<script type='text/javascript'> document.location = 'ubahformulir.php?id_anggota=$id'; </script>";
		}else{
			
        move_uploaded_file($_FILES["formulir"]["tmp_name"],"efs/formulir/".$bukti2);

		$sql = "UPDATE anggota SET
				formulir_anggota='". $bukti2."'
				WHERE id_anggota='". $id ."'";
		$ress = mysqli_query($conn, $sql);
					...
					<div class="col-lg-12">
						<form class="form-horizontal" action="<?php echo $_SERVER["PHP_SELF"];?>" method="post" enctype="multipart/form-data">
							<div class="panel panel-default">
								<div class="panel-heading"><h3>Ubah Formulir</h3></div>
